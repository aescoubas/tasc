...
// ApplyAutoSchedule processes a list of tasks and assigns virtual due times
// for "Floating" tasks.
// A task is Floating if DueAt has time 00:00:00 (or if ScheduleBlock is set).
// It respects "Fixed" tasks (DueAt has time) and stacks floating tasks
...
func ApplyAutoSchedule(tasks []models.Task, blocks map[string]config.TimeBlock) []models.Task {
	...
	for _, indices := range tasksByDay {
		...
		sort.Slice(fixed, func(i, j int) bool {
			return tasks[fixed[i]].DueAt.Before(*tasks[fixed[j]].DueAt)
		})
		...
		day := *tasks[floating[0]].DueAt // Shared day
...
func isFloating(t models.Task) bool {
	...
	// If the user adds --due tomorrow --block morning, dueAt is 00:00.
	...
	return t.DueAt.Hour() == 0 && t.DueAt.Minute() == 0 && t.DueAt.Second() == 0
}
