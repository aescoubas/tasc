...
func runImportTw(cmd *cobra.Command, args []string) {
	...
	stmt, err := tx.Prepare(`
			INSERT INTO tasks (
				...
				completed_at, due_at, active_start
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
		`)
	if err != nil {
	...
	for _, tw := range twTasks {
		...
		// 4. Insert
		res, err := stmt.Exec(title, description, tw.Project, status, createdAt, completedAt, dueAt, activeStart)
		if err != nil {
...
func parseTwDate(s string) time.Time {
	if s == "" {
		return time.Time{}
	}
